...
> ⚠️ 注意：如果挂载已存在的目录到 `/var/lib/ghost/content`，该目录必须包含 `content/adapters/storage/ghost-cos/`，否则适配器文件会被覆盖导致 Ghost 找不到适配器。建议首次使用匿名卷或空目录启动。
...
> ⚠️ 注意：`volumes` 中的 `${UPLOAD_LOCATION:-./data/ghost}` 目录如果已存在，会覆盖镜像内 `/var/lib/ghost/content` 中预装的适配器文件。首次部署请使用空目录或先移除已存在的目录。
...
> 文件名采用内容 Hash（MD5 前 12 位）+ 原始扩展名。相同内容自动去重，同名文件不会覆盖；支持中文、日文等多语言文件名（`伞耀分类-伞耀1.jpg` → `a1b2c3d4e5f6.jpg`）。
